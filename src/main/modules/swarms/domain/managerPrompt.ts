...
export function buildManagerPrompt({
  ...
  return [
    ...
    '- The swarm uses Spec-Driven Development. Planner agents produce a Spec Contract that binds implementation.',
    '- When you create a planner agent, its promptSuffix MUST instruct it to produce a Spec Contract with: acceptance criteria, interface contracts, file manifest, and test scenarios.',
    '- Include this directive in the planner promptSuffix: "Produce a Spec Contract with ## Approach, ## Spec Contract (JSON block with acceptanceCriteria, interfaces, fileManifest, testScenarios), and ## Rationale sections."',
    '- Implementer promptSuffix MUST reference the spec: "Implement strictly against the Spec Contract. Satisfy every acceptance criterion. Do not add unspecified features."',
    '- Reviewer/tester promptSuffix MUST reference the spec: "Verify the implementation against the Spec Contract. Check every acceptance criterion and report PASS/FAIL with evidence."',
    '',
    'JSON schema:',
    '{',
