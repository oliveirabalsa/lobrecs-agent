...
describe('parseReviewerVerdict', () => {
  ...
  it('falls back to approved when no marker is present', () => {
  ...
  it('falls back to approved on empty input', () => {
    expect(parseReviewerVerdict('')).toEqual({ verdict: 'approved', fallback: true })
    expect(parseReviewerVerdict(undefined)).toEqual({ verdict: 'approved', fallback: true })
  })
})
